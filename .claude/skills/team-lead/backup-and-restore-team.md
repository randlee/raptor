...
cp -r ~/.claude/tasks/$CLAUDE_CODE_TASK_LIST_ID/ "$BACKUP_PATH/tasks-cc"
echo "CC task list backed up to $BACKUP_PATH/tasks-cc"
```

> **Note**: `atm teams backup` captures the ATM sprint tasks but NOT the
> Claude Code task list at `~/.claude/tasks/$CLAUDE_CODE_TASK_LIST_ID/`.
...
Remove unexpected members (until `atm teams remove-member` ships). The
expected member list is read from `.atm.toml` in the repo root:

```python
python3 -c "
import json, os, tomllib
with open('.atm.toml', 'rb') as f: toml = tomllib.load(f)
team = os.environ['ATM_TEAM']
keep = set()
for win in toml.get('rmux', {}).get('windows', []):
    for pane in win.get('panes', []):
        env = pane.get('env', {})
        if env.get('ATM_TEAM') == team and 'ATM_IDENTITY' in env:
            keep.add(env['ATM_IDENTITY'])
...
if [ -d "$BACKUP_PATH/tasks-cc" ]; then
  cp "$BACKUP_PATH/tasks-cc/"*.json ~/.claude/tasks/$CLAUDE_CODE_TASK_LIST_ID/ 2>/dev/null || true
  MAX_ID=$(ls ~/.claude/tasks/$CLAUDE_CODE_TASK_LIST_ID/*.json 2>/dev/null \
    | xargs -I{} basename {} .json \
    | sort -n | tail -1)
  [ -n "$MAX_ID" ] && echo -n "$MAX_ID" > ~/.claude/tasks/$CLAUDE_CODE_TASK_LIST_ID/.highwatermark
