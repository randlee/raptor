...
| No `repo` column, so rows from two repositories cannot be told apart | REQ-RAP-0004 | Raptor | Sprint B.1 |
| 30 ids repeated in the corpus (title headings, in-file repeats, cross-file collisions); one ID Range claimed by two files | REQ-RAP-0006 | Consumer repository, arch-p3, task RAP-VAL-1 | Source correction; Raptor only reports each repeat as a diagnostic |
| `hitl/**` excluded and `database/schema` omitted from the scan inventory | REQ-RAP-0008 | Consumer repository, arch-p3, task RAP-VAL-1 | `.raptor/sources.toml`, `.raptor/routing.toml` in that repository |
| Database pointer (URL, or path / environment variable for test) not declared in `.raptor/` | REQ-RAP-0008 | Product | Not in Phase B |
| Product and module columns | REQ-RAP-0004 | Product | Not in Phase B; `domain` stands in for module |
