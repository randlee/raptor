...
| render to new path | preserve origin including initial path | new repository-relative path registered for the same document by A5 | compute new hash; parent is prior hash | document/artifact composite keys unchanged; A5's durable journal commits output, then manifest binding, then retryable store put and recovers any interrupted intermediate state |
